        ...
        {activeSection && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mt-10 rounded-[2rem] border border-neutral-700/40 bg-[#211d1c]/95 p-5 shadow-[0_24px_80px_-42px_rgba(0,0,0,.9)] lg:p-7">
          ...
          <div className="space-y-8">
            ...
            {activeSection === "dialogue" && lessonMeta?.dialogue?.length > 0 && (() => {
              ...
              const currentLine = dialogue[dialogueIndex] || dialogue[0];
              ...
              const isPlayerTurn = currentLine.speaker === playerRole;
              return <section id="dialogue" className="scroll-mt-24 overflow-hidden rounded-2xl border border-neutral-700/40 bg-gradient-to-br from-[#211d1c] to-orange-950/20 p-5 lg:p-7">
                <div className="relative mb-6 overflow-hidden rounded-2xl border border-orange-500/20 bg-orange-500/10 p-5"><div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-orange-500/20 blur-2xl" /><div className="relative flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Conversation à deux</p><h3 className="mt-2 font-heading text-2xl font-bold text-white">Choisis ton rôle</h3><p className="mt-2 text-sm leading-6 text-neutral-300">Tu incarnes un personnage. Quand c’est ton tour, dis la réplique à voix haute.</p></div><MessageCircle size={32} className="shrink-0 text-orange-300" /></div><div className="mt-4 flex flex-wrap gap-2">{speakers.map((speaker) => <button key={speaker} type="button" onClick={() => { setDialogueRole(speaker); setDialogueIndex(0); setDialogueSpoken(false); setShowDialogueTranslation(false); }} className={`rounded-xl border px-3 py-2 text-sm font-semibold transition ${playerRole === speaker ? "border-orange-400 bg-orange-500 text-white" : "border-neutral-700/40 bg-neutral-900/60 text-neutral-300 hover:border-orange-500/50"}`}>Je suis {speaker}</button>)}</div><div className="mt-5 h-2 overflow-hidden rounded-full bg-neutral-900/70"><motion.div className="h-full rounded-full bg-orange-500" animate={{ width: `${((dialogueIndex + 1) / dialogue.length) * 100}%` }} /></div><p className="mt-2 text-right text-xs font-semibold text-orange-300">Réplique {dialogueIndex + 1} / {dialogue.length}</p></div>
                <motion.article key={dialogueIndex} initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className={`relative max-w-2xl rounded-[2rem] border border-neutral-700/40 p-6 shadow-[0_24px_70px_-35px_rgba(249,115,22,.45)] lg:p-8 ${isPlayerTurn ? "ml-auto bg-orange-500/15" : "mr-auto bg-neutral-900/80"}`}><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-orange-500/15 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-orange-300">{currentLine.speaker}</span><span className={`text-xs font-bold ${isPlayerTurn ? "text-orange-300" : "text-neutral-500"}`}>{isPlayerTurn ? "À toi" : "Écoute"}</span></div><p className="mt-6 text-center font-heading text-2xl font-bold leading-relaxed text-white lg:text-3xl">{currentLine.text}</p>{currentLine.phonetic_simple && <p className="mt-4 text-center font-mono text-sm text-orange-300">{currentLine.phonetic_simple}</p>}<div className="mt-6 flex justify-center gap-2"><button type="button" onClick={() => speak(currentLine.text)} className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-orange-950/30 transition hover:-translate-y-0.5 hover:bg-orange-400"><Volume2 size={17} />{isPlayerTurn ? "Écouter puis répéter" : "Écouter"}</button>{isPlayerTurn && <button type="button" onClick={() => setDialogueSpoken(true)} className={`rounded-xl border px-4 py-3 text-sm font-bold transition ${dialogueSpoken ? "border-emerald-400 bg-emerald-500/15 text-emerald-300" : "border-neutral-700/40 bg-neutral-900 text-neutral-300 hover:border-orange-500/50"}`}>{dialogueSpoken ? "Réplique dite ✓" : "J’ai dit la réplique"}</button>}</div>{showDialogueTranslation && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-6 rounded-2xl border border-neutral-700/40 bg-neutral-800/80 p-4 text-center"><p className="text-xs font-bold uppercase tracking-wider text-neutral-500">Traduction</p><p className="mt-2 text-sm leading-6 text-neutral-200">{currentLine.translation}</p></motion.div>}</motion.article>
                <div className="mx-auto mt-5 flex max-w-2xl flex-wrap justify-center gap-3"><button type="button" onClick={() => setShowDialogueTranslation((value) => !value)} className="rounded-xl border border-neutral-700/40 bg-neutral-900 px-4 py-3 text-sm font-semibold text-neutral-300 transition hover:border-orange-500/50 hover:text-white">{showDialogueTranslation ? "Masquer la traduction" : "Voir la traduction"}</button><button type="button" disabled={isPlayerTurn && !dialogueSpoken} onClick={() => { setShowDialogueTranslation(false); setDialogueSpoken(false); setDialogueIndex((index) => isLastLine ? 0 : index + 1); }} className="rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-orange-950/30 transition hover:-translate-y-0.5 hover:bg-orange-400 disabled:cursor-not-allowed disabled:opacity-40">{isLastLine ? "Rejouer la conversation" : "Réplique suivante →"}</button></div>
              </section>;
            })()}
