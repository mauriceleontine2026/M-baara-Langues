      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-6 lg:px-8 lg:py-10">
        ...
        <AnimatePresence mode="wait">
          {phase === "learn" && (
            <motion.div key="learn-grid" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full">
              <div className="mb-5 flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Apprentissage / Vocabulaire</p><h2 className="mt-1 font-heading text-2xl font-bold text-foreground">Découvre les mots de la leçon</h2></div><span className="rounded-full border border-border bg-card px-3 py-1.5 text-xs font-semibold text-muted-foreground">{items.length} cartes</span></div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {items.map((item, index) => <motion.article key={item.word_id || `${item.word}-${index}`} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.025, 0.35) }} className="flex min-h-[270px] flex-col rounded-2xl border border-border bg-card p-4 shadow-[0_16px_40px_-28px_rgba(0,0,0,0.35)] transition hover:-translate-y-1 hover:border-orange-500/50">
                  <div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl bg-orange-500/10 text-orange-400"><LanguageFlag language={language} size="sm" /></span><span className="text-xs text-muted-foreground">{String(index + 1).padStart(2, "0")}</span></div>
                  <div className="flex flex-1 flex-col justify-center py-5"><h3 className="text-xl font-bold text-foreground">{item.translation_fr || item.word}</h3><p className="mt-2 text-lg font-medium text-muted-foreground">{item.word}</p>{item.phonetic && <p className="mt-2 font-mono text-xs text-orange-500">/{item.phonetic}/</p>}{item.example_target && <p className="mt-3 line-clamp-2 text-xs italic text-muted-foreground">“{item.example_target}”</p>}</div>
                  <div className="flex gap-2"><button type="button" onClick={() => playAudio(item)} className="flex-1 rounded-xl bg-orange-500 px-3 py-2.5 text-sm font-bold text-white transition hover:bg-orange-400"><Volume2 size={15} className="mr-1.5 inline" />Écouter</button><button type="button" aria-label={`Écouter ${item.word}`} onClick={() => playAudio(item)} className="grid h-10 w-10 place-items-center rounded-xl border border-border bg-secondary text-foreground transition hover:bg-muted"><Volume2 size={16} /></button></div>
                </motion.article>)}
              </div>
        ...
        {activeSection && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mt-10 rounded-[2rem] border border-neutral-700/40 bg-[#211d1c]/95 p-5 shadow-[0_24px_80px_-42px_rgba(0,0,0,.9)] lg:p-7">
          <div className="mb-6 flex items-end justify-between gap-4 border-b border-border/70 pb-4">
            ...
            <span className="rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground">{items.length} mots · {lessonMeta?.exercises?.length || 0} défis</span>
          </div>
          <div className="space-y-8">
            ...
            {activeSection === "dialogue" && lessonMeta?.dialogue?.length > 0 && (() => {
              ...
              return <section id="dialogue" className="scroll-mt-24 overflow-hidden rounded-2xl border border-border bg-card p-5 lg:p-7">
                <div className="relative mb-6 overflow-hidden rounded-2xl border border-orange-500/20 bg-orange-500/10 p-5"><div className="relative flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Conversation à deux</p><h3 className="mt-2 font-heading text-2xl font-bold text-foreground">Dialogue complet</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Les deux personnes dialoguent ensemble. Choisis ton rôle, écoute chaque voix et répète tes répliques.</p></div><MessageCircle size={32} className="shrink-0 text-orange-300" /></div><div className="mt-4 flex flex-wrap gap-2">{speakers.map((speaker) => <button key={speaker} type="button" onClick={() => setDialogueRole(speaker)} className={`rounded-xl border px-3 py-2 text-sm font-semibold transition ${playerRole === speaker ? "border-orange-400 bg-orange-500 text-white" : "border-border bg-card text-muted-foreground hover:border-orange-500/50"}`}>Je suis {speaker}</button>)}</div><div className="mt-5 h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full w-full rounded-full bg-orange-500" /></div><p className="mt-2 text-right text-xs font-semibold text-orange-300">{dialogue.length} répliques · {speakers.length} interlocuteurs</p></div>
                <div className="space-y-4">{dialogue.map((line, index) => { const isPlayerLine = line.speaker === playerRole; return <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .06, .4) }} key={`${line.speaker}-${index}`} className={`flex ${isPlayerLine ? "justify-end" : "justify-start"}`}><div className={`max-w-2xl rounded-[1.5rem] border p-4 shadow-sm ${isPlayerLine ? "border-orange-500/30 bg-orange-500/15" : "border-border bg-background"}`}><div className="flex items-center justify-between gap-4"><span className="text-xs font-bold uppercase tracking-wider text-orange-400">{line.speaker}</span><span className="text-[10px] text-muted-foreground">{isPlayerLine ? "Ton rôle" : "Interlocuteur"}</span></div><p className="mt-3 text-base font-medium leading-7 text-foreground">{line.text}</p>{line.phonetic_simple && <p className="mt-2 font-mono text-xs text-orange-400">{line.phonetic_simple}</p>}<div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={() => speak(line.text)} className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3 py-2 text-xs font-bold text-white transition hover:bg-orange-400"><Volume2 size={14} />Écouter</button><button type="button" onClick={() => setShowDialogueTranslation((value) => !value)} className="rounded-xl border border-border bg-card px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:border-orange-500/50">{showDialogueTranslation ? "Masquer" : "Traduction"}</button></div>{showDialogueTranslation && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 border-t border-border pt-3 text-sm text-muted-foreground">{line.translation}</motion.p>}</div></motion.article>; })}</div>
                <div className="mt-6 flex justify-center"><button type="button" onClick={() => { setDialogueRole(null); setDialogueIndex(0); setShowDialogueTranslation(false); }} className="rounded-xl border border-border bg-card px-5 py-3 text-sm font-bold text-muted-foreground transition hover:border-orange-500/50 hover:text-foreground">Rejouer la conversation</button></div>
              </section>;
            })()}
            {activeSection === "exercices" && lessonMeta?.exercises?.length > 0 && <section id="exercices" className="scroll-mt-24 overflow-hidden rounded-2xl border border-border bg-card p-6"><div className="relative flex flex-col items-center text-center"><div className="relative grid h-16 w-16 place-items-center rounded-2xl bg-orange-500 text-white shadow-[0_0_35px_rgba(249,115,22,.35)]"><Target size={30} /></div><p className="mt-4 text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Évaluation de la leçon</p><h3 className="mt-2 font-heading text-2xl font-bold text-foreground">Prêt pour l’examen ?</h3><p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Les questions restent cachées jusqu’au défi. Lance l’examen, gagne des points et mesure ta maîtrise.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{[...new Set(lessonMeta.exercises.map((exercise) => exercise.type || "Exercice"))].map((type) => <span key={type} className="rounded-full border border-border bg-background px-3 py-1.5 text-xs font-semibold text-muted-foreground">{type}</span>)}<span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-1.5 text-xs font-semibold text-orange-300">{lessonMeta.exercises.length} exercices</span></div><Link to={`/examen/${langCode}/${lessonNum}`} className="mt-6 rounded-xl bg-orange-500 px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-orange-950/30 transition hover:-translate-y-0.5 hover:bg-orange-400">Lancer le défi →</Link></div></section>}
          </div>
        </motion.section>}
      </main>
    </div>
