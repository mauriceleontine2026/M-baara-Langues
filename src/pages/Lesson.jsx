  ...
  return (
    <div className="min-h-screen bg-background flex flex-col" style={{ backgroundImage: `radial-gradient(circle at 15% 0%, ${language.color}18, transparent 32%), radial-gradient(circle at 90% 18%, ${language.color}12, transparent 28%)` }}>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-6 lg:px-8 lg:py-10">
        ...
        {activeSection && (lessonMeta?.learning_objectives?.length > 0 || lessonMeta?.phonetic_focus || lessonMeta?.common_phrases?.length > 0 || lessonMeta?.grammar_points?.length > 0 || lessonMeta?.cultural_notes?.length > 0) && (
          <div className="mb-8 grid gap-4 lg:grid-cols-2">
            {activeSection === "objectifs" && lessonMeta.learning_objectives?.length > 0 && (
              <section id="objectifs" className="scroll-mt-24 rounded-2xl border border-border/40 bg-card p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <div className="mb-5 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Ton itinéraire</p><h2 className="mt-1 flex items-center gap-2 font-semibold text-foreground"><Target size={18} className="text-orange-400" />Objectifs à débloquer</h2></div><span className="rounded-full bg-orange-500/10 px-3 py-1.5 text-xs font-bold text-orange-300">{lessonMeta.learning_objectives.length} étapes</span></div>
                <div className="grid gap-3 sm:grid-cols-2">{lessonMeta.learning_objectives.map((objective, index) => { const done = completedObjectives.includes(index); return <motion.button type="button" onClick={() => setCompletedObjectives((current) => done ? current.filter((value) => value !== index) : [...current, index])} initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * .08 }} key={objective} className={`group relative overflow-hidden rounded-2xl border p-4 text-left transition hover:-translate-y-1 ${done ? "border-emerald-400/50 bg-emerald-500/10" : "border-border/40 bg-secondary/60 hover:border-orange-500/50"}`}><div className="absolute -right-8 -top-8 h-20 w-20 rounded-full bg-orange-500/10 blur-xl transition group-hover:bg-orange-500/25" /><span className={`relative grid h-8 w-8 place-items-center rounded-xl text-xs font-black text-white ${done ? "bg-emerald-500" : "bg-orange-500"}`}>{done ? "✓" : index + 1}</span><p className="relative mt-3 text-sm leading-6 text-foreground">{objective}</p><span className={`mt-3 block text-xs font-semibold ${done ? "text-emerald-300" : "text-orange-300"}`}>{done ? "Objectif maîtrisé" : "Marquer comme acquis"}</span></motion.button>; })}</div>
              </section>
            )}
            {activeSection === "phonétique" && lessonMeta.phonetic_focus && (
              <section id="phonétique" className="scroll-mt-24 rounded-2xl border border-border/40 bg-card p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <h2 className="flex items-center gap-2 font-semibold text-foreground"><Volume2 size={18} className="text-orange-400" />Focus phonétique</h2>
                <div className="mt-4 grid gap-3 sm:grid-cols-2"><motion.article whileHover={{ y: -4 }} className="rounded-2xl border border-orange-500/25 bg-gradient-to-br from-orange-500/15 to-orange-950/20 p-5"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wider text-orange-400">À retenir</p><button type="button" onClick={() => speak(lessonMeta.phonetic_focus.key_sounds)} className="grid h-9 w-9 place-items-center rounded-lg bg-orange-500/20 text-orange-300 hover:bg-orange-500 hover:text-white"><Volume2 size={16} /></button></div><p className="text-sm leading-6 text-neutral-200">{lessonMeta.phonetic_focus.key_sounds}</p></motion.article>{lessonMeta.phonetic_focus.common_pitfalls && <motion.article whileHover={{ y: -4 }} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-5"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wider text-neutral-400">À surveiller</p><span className="text-2xl text-amber-400">!</span></div><p className="text-sm leading-6 text-neutral-300">{lessonMeta.phonetic_focus.common_pitfalls}</p></motion.article>}</div>
              </section>
            )}
            {activeSection === "phrases" && lessonMeta.common_phrases?.length > 0 && (
              <section id="phrases" className="scroll-mt-24 rounded-2xl border border-border/40 bg-card p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <h2 className="flex items-center gap-2 font-semibold text-foreground"><MessageCircle size={18} className="text-orange-400" />Phrases utiles</h2>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {lessonMeta.common_phrases.map((phrase, index) => <motion.article whileHover={{ y: -4 }} key={phrase.phrase_id || phrase.original} className="group rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4 transition hover:border-orange-500/50"><div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-wider text-orange-400">Phrase {index + 1}</span><button type="button" aria-label="Écouter la phrase" onClick={() => speak(phrase.original)} className="grid h-8 w-8 place-items-center rounded-lg bg-orange-500/15 text-orange-300 transition hover:bg-orange-500 hover:text-white"><Volume2 size={15} /></button></div><p className="mt-3 text-base font-bold text-white">{phrase.original}</p>{phrase.phonetic_simple && <p className="mt-1 font-mono text-xs text-orange-300">{phrase.phonetic_simple}</p>}<p className="mt-2 text-sm text-neutral-300">{phrase.translation}</p>{phrase.context && <p className="mt-3 border-t border-neutral-700/40 pt-3 text-xs leading-5 text-neutral-500">{phrase.context}</p>}</motion.article>)}
                </div>
              </section>
            )}
            {activeSection === "grammaire" && lessonMeta.grammar_points?.length > 0 && (
              <section id="grammaire" className="scroll-mt-24 rounded-2xl border border-border/40 bg-card p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <h2 className="flex items-center gap-2 font-semibold text-foreground"><Sparkles size={18} className="text-orange-400" />Points de grammaire</h2>
                <div className="mt-4 grid gap-4">
                  {lessonMeta.grammar_points.map((point, pointIndex) => { const isOpen = openGrammarPoint === pointIndex; return <motion.article initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: pointIndex * .1 }} key={point.concept} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-5"><button type="button" onClick={() => setOpenGrammarPoint(isOpen ? -1 : pointIndex)} className="flex w-full items-start gap-3 text-left"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-orange-500 text-sm font-black text-white">{pointIndex + 1}</span><span className="min-w-0 flex-1"><h3 className="text-base font-semibold text-white">{point.concept}</h3><span className="mt-2 block text-xs font-semibold text-orange-300">{isOpen ? "Réduire la règle" : "Ouvrir l’explication →"}</span></span></button>{isOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-4 border-t border-neutral-700/40 pt-4"><p className="text-sm leading-6 text-neutral-300">{point.explanation}</p>{point.rules?.length > 0 && <div className="mt-5 grid gap-2">{point.rules.map((rule, index) => <p key={rule} className="rounded-xl bg-neutral-800/80 p-3 text-xs leading-5 text-neutral-300"><strong className="mr-1 text-orange-400">Règle {index + 1}.</strong>{rule}</p>)}</div>}{point.examples?.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2">{point.examples.map((example) => <p key={example.structure} className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-3 text-xs text-neutral-300"><strong className="block text-white">{example.structure}</strong><span className="mt-1 block">{example.meaning}</span></p>)}</div>}</motion.div>}</motion.article>; })}
                </div>
              </section>
            )}
            {activeSection === "culture" && lessonMeta.cultural_notes?.length > 0 && (
              <section id="culture" className="scroll-mt-24 rounded-2xl border border-border/40 bg-card p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <h2 className="flex items-center gap-2 font-semibold text-foreground"><Sparkles size={18} className="text-orange-400" />Notes culturelles</h2>
                <div className="mt-4 grid gap-3">{lessonMeta.cultural_notes.map((note, index) => <motion.article whileHover={{ scale: 1.01 }} key={note} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4 transition hover:border-orange-500/50"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-orange-500/15 text-lg text-orange-300">✦</span><span className="text-xs font-bold uppercase tracking-wider text-orange-400">Repère culturel {index + 1}</span></div><p className="mt-3 text-sm leading-6 text-neutral-300">{note}</p></motion.article>)}</div>
              </section>
            )}
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
          {phase === "learn" && (
            <motion.div key="learn-grid" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full">
              <div className="mb-5 flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Apprentissage / Vocabulaire</p><h2 className="mt-1 font-heading text-2xl font-bold text-foreground">Découvre les mots de la leçon</h2></div><span className="rounded-full border border-border/40 bg-secondary px-3 py-1.5 text-xs font-semibold text-muted-foreground">{items.length} cartes</span></div>
        ...
        {activeSection && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mt-10 rounded-[2rem] border border-border/40 bg-card/95 p-5 shadow-[0_24px_80px_-42px_rgba(0,0,0,.9)] lg:p-7">
          <div className="mb-6 flex items-end justify-between gap-4 border-b border-border/70 pb-4">
            <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Espace d’apprentissage</p><h2 className="mt-1 font-heading text-2xl font-bold text-foreground">{activeSection === "vocabulaire" ? "Les mots à maîtriser" : activeSection === "dialogue" ? "Parler en situation" : activeSection === "exercices" ? "Le défi de la leçon" : "Les clés de la leçon"}</h2></div>
            <span className="rounded-full border border-border/40 bg-secondary px-3 py-1.5 text-xs text-muted-foreground">{items.length} mots · {lessonMeta?.exercises?.length || 0} défis</span>
          </div>
          <div className="space-y-8">
            {activeSection === "vocabulaire" && <section id="vocabulaire-complet" className="scroll-mt-24">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items.map((item, index) => <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.025, 0.3) }} key={item.word_id || `${item.word}-${index}`} className="group rounded-2xl border border-border/40 bg-card/70 p-4 transition duration-300 hover:-translate-y-1 hover:border-orange-500/50 hover:shadow-[0_18px_45px_-28px_rgba(249,115,22,.7)]"><div className="flex items-start justify-between gap-2"><h4 className="font-heading text-lg font-bold text-foreground group-hover:text-orange-300">{item.word}</h4><span className="rounded-full bg-orange-500/10 px-2 py-1 text-[10px] font-bold text-orange-400">#{index + 1}</span></div>{item.phonetic && <p className="mt-2 font-mono text-xs text-orange-300">{item.phonetic}</p>}{item.phonetic_simple && <p className="text-xs text-muted-foreground">{item.phonetic_simple}</p>}<p className="mt-2 text-sm text-muted-foreground">{item.translation_fr}</p>{item.example_target && <div className="mt-3 rounded-xl bg-orange-500/5 p-3 text-xs"><p className="italic text-foreground">{item.example_target}</p><p className="mt-1 text-muted-foreground">{item.example_fr}</p></div>}</motion.article>)}</div>
            </section>}
            {activeSection === "dialogue" && lessonMeta?.dialogue?.length > 0 && (() => {
              ...
              return <section id="dialogue" className="scroll-mt-24 overflow-hidden rounded-2xl border border-border/40 bg-gradient-to-br from-card to-orange-950/20 p-5 lg:p-7">
                ...
                <div className="space-y-4">{dialogue.map((line, index) => { const isPlayerLine = line.speaker === playerRole; return <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .06, .4) }} key={`${line.speaker}-${index}`} className={`flex ${isPlayerLine ? "justify-end" : "justify-start"}`}><div className={`max-w-2xl rounded-[1.5rem] border p-4 shadow-sm ${isPlayerLine ? "border-orange-500/30 bg-orange-500/15" : "border-border/40 bg-card/80"}`}><div className="flex items-center justify-between gap-4"><span className="text-xs font-bold uppercase tracking-wider text-orange-300">{line.speaker}</span><span className="text-[10px] text-muted-foreground">{isPlayerLine ? "Ton rôle" : "Interlocuteur"}</span></div><p className="mt-3 text-base font-medium leading-7 text-foreground">{line.text}</p>{line.phonetic_simple && <p className="mt-2 font-mono text-xs text-orange-300">{line.phonetic_simple}</p>}<div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={() => speak(line.text)} className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3 py-2 text-xs font-bold text-white transition hover:bg-orange-400"><Volume2 size={14} />Écouter</button><button type="button" onClick={() => setShowDialogueTranslation((value) => !value)} className="rounded-xl border border-border/40 bg-secondary px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:border-orange-500/50">{showDialogueTranslation ? "Masquer" : "Traduction"}</button></div>{showDialogueTranslation && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 border-t border-border/40 pt-3 text-sm text-muted-foreground">{line.translation}</motion.p>}</div></motion.article>; })}</div>
            ...
            {activeSection === "exercices" && lessonMeta?.exercises?.length > 0 && <section id="exercices" className="scroll-mt-24 overflow-hidden rounded-2xl border border-border/40 bg-gradient-to-br from-card to-orange-950/20 p-6"><div className="relative flex flex-col items-center text-center"><div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-orange-500/15 blur-2xl" /><div className="relative grid h-16 w-16 place-items-center rounded-2xl bg-orange-500 text-white shadow-[0_0_35px_rgba(249,115,22,.35)]"><Target size={30} /></div><p className="mt-4 text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Évaluation de la leçon</p><h3 className="mt-2 font-heading text-2xl font-bold text-foreground">Prêt pour l’examen ?</h3><p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Les questions restent cachées jusqu’au défi. Lance l’examen, gagne des points et mesure ta maîtrise.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{[...new Set(lessonMeta.exercises.map((exercise) => exercise.type || "Exercice"))].map((type) => <span key={type} className="rounded-full border border-border/40 bg-secondary px-3 py-1.5 text-xs font-semibold text-muted-foreground">{type}</span>)}<span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-1.5 text-xs font-semibold text-orange-300">{lessonMeta.exercises.length} exercices</span></div><Link to={`/examen/${langCode}/${lessonNum}`} className="mt-6 rounded-xl bg-orange-500 px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-orange-950/30 transition hover:-translate-y-0.5 hover:bg-orange-400">Lancer le défi →</Link></div></section>}
          </div>
        </motion.section>}
      </main>
    </div>
