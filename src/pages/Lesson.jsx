  ...
  return (
    <div className="min-h-screen bg-background flex flex-col" style={{ backgroundImage: `radial-gradient(circle at 15% 0%, ${language.color}18, transparent 32%), radial-gradient(circle at 90% 18%, ${language.color}12, transparent 28%)` }}>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-6 lg:px-8 lg:py-10">
          ...
          <div className="mb-8 grid gap-4 lg:grid-cols-2">
            {activeSection === "objectifs" && lessonMeta.learning_objectives?.length > 0 && (
              <section id="objectifs" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                <h2 className="flex items-center gap-2 font-semibold text-white"><Target size={18} className="text-orange-400" />Objectifs</h2>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">{lessonMeta.learning_objectives.map((objective, index) => <article key={objective} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4 transition hover:border-orange-500/50"><span className="text-xs font-bold text-orange-400">0{index + 1}</span><p className="mt-2 text-sm leading-6 text-neutral-200">{objective}</p></article>)}</div>
              </section>
            )}
            {activeSection === "phonétique" && lessonMeta.phonetic_focus && (
              <section id="phonétique" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2"><article className="rounded-2xl border border-orange-500/25 bg-orange-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wider text-orange-400">À retenir</p><p className="mt-2 text-sm leading-6 text-neutral-200">{lessonMeta.phonetic_focus.key_sounds}</p></article>{lessonMeta.phonetic_focus.common_pitfalls && <article className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4"><p className="text-xs font-bold uppercase tracking-wider text-neutral-400">À surveiller</p><p className="mt-2 text-sm leading-6 text-neutral-300">{lessonMeta.phonetic_focus.common_pitfalls}</p></article>}</div>
              </section>
            )}
            {activeSection === "phrases" && lessonMeta.common_phrases?.length > 0 && (
              <section id="phrases" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-5 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.9)]">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {lessonMeta.common_phrases.map((phrase, index) => <article key={phrase.phrase_id || phrase.original} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4 transition hover:-translate-y-0.5 hover:border-orange-500/50"><span className="text-[10px] font-bold uppercase tracking-wider text-orange-400">Phrase {index + 1}</span><p className="mt-2 text-base font-bold text-white">{phrase.original}</p>{phrase.phonetic_simple && <p className="mt-1 font-mono text-xs text-orange-300">{phrase.phonetic_simple}</p>}<p className="mt-2 text-sm text-neutral-300">{phrase.translation}</p>{phrase.context && <p className="mt-3 border-t border-neutral-700/40 pt-3 text-xs leading-5 text-neutral-500">{phrase.context}</p>}</article>)}
                </div>
              </section>
            )}
        ...
        {activeSection && <section className="mt-10 rounded-[2rem] border border-border/70 bg-card/80 p-5 shadow-sm lg:p-7">
          ...
          <div className="space-y-8">
            ...
            {activeSection === "dialogue" && lessonMeta?.dialogue?.length > 0 && <section id="dialogue" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-5"><h3 className="mb-4 flex items-center gap-2 font-semibold text-white"><MessageCircle size={18} className="text-orange-400" />Dialogue</h3><div className="space-y-3">{lessonMeta.dialogue.map((line, index) => <article key={`${line.speaker}-${index}`} className={`max-w-3xl rounded-2xl border border-neutral-700/40 p-4 ${index % 2 === 0 ? "bg-orange-500/10" : "ml-auto bg-neutral-900/70"}`}><p className="text-xs font-bold uppercase tracking-wider text-orange-400">{line.speaker}</p><p className="mt-2 font-medium text-white">{line.text}</p>{line.phonetic_simple && <p className="mt-1 font-mono text-xs text-orange-300">{line.phonetic_simple}</p>}<p className="mt-2 text-sm text-neutral-300">{line.translation}</p></article>)}</div></section>}
            {activeSection === "exercices" && lessonMeta?.exercises?.length > 0 && <section id="exercices" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h3 className="flex items-center gap-2 font-semibold text-white"><Target size={18} className="text-orange-400" />Exercices de la leçon</h3>{lessonMeta.module?.id && <Link to={`/exercice/${langCode}/${lessonMeta.module.id}`} className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-orange-400">Lancer l’examen</Link>}</div><div className="grid gap-3 lg:grid-cols-2">{lessonMeta.exercises.map((exercise, index) => <article key={exercise.exercise_id || index} className="rounded-2xl border border-neutral-700/40 bg-neutral-900/60 p-4 transition hover:border-orange-500/50"><p className="text-xs font-bold uppercase tracking-wider text-orange-400">Exercice {index + 1} · {exercise.type}</p><p className="mt-2 font-medium text-white">{exercise.question || exercise.sentence_with_blank}</p>{exercise.options?.length > 0 && <ul className="mt-2 space-y-1 text-sm text-neutral-300">{exercise.options.map((option) => <li key={option} className="rounded-lg bg-neutral-800 px-3 py-2">{option}</li>)}</ul>}{exercise.hint && <p className="mt-2 text-xs text-neutral-500">Indice : {exercise.hint}</p>}<p className="mt-3 border-t border-neutral-700/40 pt-2 text-sm text-orange-300">Réponse : {exercise.correct_answer}</p>{exercise.explanation && <p className="mt-1 text-xs text-neutral-400">{exercise.explanation}</p>}</article>)}</div></section>}
          </div>
        </section>}
      </main>
    </div>
