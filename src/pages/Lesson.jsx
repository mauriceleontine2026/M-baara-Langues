      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-6 lg:px-8 lg:py-10">
        ...
        {activeSection && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mt-10 rounded-[2rem] border border-neutral-700/40 bg-[#211d1c]/95 p-5 shadow-[0_24px_80px_-42px_rgba(0,0,0,.9)] lg:p-7">
          ...
          <div className="space-y-8">
            ...
            {activeSection === "dialogue" && lessonMeta?.dialogue?.length > 0 && <section id="dialogue" className="scroll-mt-24"><div className="mb-4 rounded-2xl bg-orange-500/10 p-4 text-sm text-neutral-300">Lis le dialogue à voix haute, puis écoute chaque réplique avec la synthèse vocale de ton navigateur.</div><div className="space-y-4">{lessonMeta.dialogue.map((line, index) => <motion.article initial={{ opacity: 0, x: index % 2 ? 18 : -18 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.08 }} key={`${line.speaker}-${index}`} className={`max-w-3xl rounded-2xl border border-neutral-700/40 p-4 shadow-sm ${index % 2 === 0 ? "bg-orange-500/10" : "ml-auto bg-neutral-900/80"}`}><p className="text-xs font-bold uppercase tracking-wider text-orange-400">{line.speaker}</p><p className="mt-2 text-base font-medium leading-7 text-white">{line.text}</p>{line.phonetic_simple && <p className="mt-2 font-mono text-xs text-orange-300">{line.phonetic_simple}</p>}<p className="mt-2 border-t border-neutral-700/40 pt-2 text-sm text-neutral-400">{line.translation}</p></motion.article>)}</div></section>}
            {activeSection === "exercices" && lessonMeta?.exercises?.length > 0 && <section id="exercices" className="scroll-mt-24 rounded-2xl border border-neutral-700/40 bg-[#211d1c] p-6"><div className="flex flex-col items-center text-center"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-orange-500/15 text-orange-400"><Target size={30} /></div><p className="mt-4 text-xs font-bold uppercase tracking-[0.2em] text-orange-400">Évaluation de la leçon</p><h3 className="mt-2 font-heading text-2xl font-bold text-white">Prêt pour l’examen ?</h3><p className="mt-2 max-w-lg text-sm leading-6 text-neutral-400">Les questions et les réponses restent cachées pendant la préparation. Lance l’examen pour répondre, être corrigé et obtenir ton score.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{[...new Set(lessonMeta.exercises.map((exercise) => exercise.type || "Exercice"))].map((type) => <span key={type} className="rounded-full border border-neutral-700/40 bg-neutral-900 px-3 py-1.5 text-xs font-semibold text-neutral-300">{type}</span>)}<span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-1.5 text-xs font-semibold text-orange-300">{lessonMeta.exercises.length} exercices</span></div><Link to={`/examen/${langCode}/${lessonNum}`} className="mt-6 rounded-xl bg-orange-500 px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-orange-950/30 transition hover:bg-orange-400">Lancer l’examen →</Link></div></section>}
          </div>
        </motion.section>}
      </main>
    </div>
